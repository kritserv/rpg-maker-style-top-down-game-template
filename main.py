...
from src import clock_tick, curr_fps,\
	...
	Cursor, Menu, Debugger, TextBox, Renderer
...
async def main():
	...
	camera = Camera(player, top_down_map, Renderer(load_asset_from_json()))
	...
	while running:
		...
		if game_state == "title_screen_menu":
			...
			screen.fill(darkblue)
			camera.draw(screen, scene_manager.current_scene, pixel_size)
			black_bar.draw_if_set(curr_width, curr_height, ratio)
			if event.draw_text_box:
